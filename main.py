...
db_briefings = banco["briefings_Eurochem"]  
...
with tab_chatbot:  
    st.header("Chat Virtual Eurochem")
    st.caption("Pergunte qualquer coisa sobre as diretrizes e informações da Eurochem")
    ...
    if prompt := st.chat_input("Como posso ajudar?"):
        ...
        contexto = f"""
        ###BEGIN SYSTEM PROMPT####
        Você é um assistente virtual especializado na Eurochem
        Baseie todas as suas respostas nas diretrizes oficiais da Eurochem
        ...
        ###BEGIN DIRETRIZES DA EMPRESA###
        {conteudo}
        ###END DIRETRIZES DA EMPRESA###
        """
    ...
    with col1:
        ...
        if st.button("Gerar Especificações", key="gen_visual"):
            with st.spinner('Criando guia de estilo...'):
                prompt = f"""
                Você é um designer que trabalha para a Macfor Marketing digital e você deve gerar conteúdo criativo para o cliente Eurochem.
...
with tab_briefing:
    st.header("Gerador de Briefing Eurochem")
    st.caption("Crie briefings completos para diferentes áreas de atuação da Eurochem")
    
    # Conexão com MongoDB para briefings
    db_briefings = client2['briefings_Eurochem']
    ...
    with tab_new:
        ...
        def criar_campo_selecionavel(rotulo, tipo="text_area", opcoes=None, padrao=None, key_suffix=""):
            ...
            if auto_preencher:
                ...
                prompt = f"Com base no seguinte contexto:\n{contexto}\n\n E o objetivo do briefing {objetivo_geral} \n\nPreencha o campo '{rotulo}' para um briefing do tipo {tipo_briefing} no Eurochem. Retorne APENAS o valor para o campo, sem comentários ou formatação adicional."
        ...
        if st.button("🔄 Gerar Briefing Completo", type="primary"):
            with st.spinner('Construindo briefing profissional...'):
                try:
                    ...
                    prompt_parts = [
                        f"# BRIEFING {tipo_briefing.upper()} - Eurochem",
    ...
    with tab_saved:
        ...
        collection_briefings = client2.briefings_Eurochem.briefings  # Ajuste aqui
...
with tab_resumo:
    ...
    st.caption("Resuma textos longos mantendo o alinhamento com as diretrizes da Eurochem")
            ...
            if not texto_original.strip():
                ...
                with st.spinner("Processando resumo..."):
                    try:
                        ...
                        prompt = f"""
                        Crie um resumo profissional deste texto para a Eurochem,
                        ...
                        - Mantenha o tom profissional da Eurochem
    ...
    with tab_briefing_gerados:
        st.header("📚 Briefings Gerados - Eurochem")
        st.markdown("---")
